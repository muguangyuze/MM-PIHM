  ...
  //printf(" river9 flux = %d \n", k-1);
    ...
    if (pihmRTcopy->river[i].up1 < 0)
		...
		else
		{
			CD->Flux[k].nodeup = i + 2 * nelem + nriver + 1;
      // 01.12 fix bug
			//CD->Flux[k].nodelo = i + 2 * nelem + nriver;
      // 02.21 fix critical bug
      //CD->Flux[k].nodelo = pihmRTcopy->river[i].up1;
      CD->Flux[k].nodelo = 2 * nelem + nriver + pihmRTcopy->river[i].up1;
      //printf(" upstream riv [i=%d] = %d \n", i+1, pihmRTcopy->river[i].up1);
      // 01.12 fix bug
		}
    // 02.21 fix tributary up2 bug
    ...
    {
      CD->Flux[k].node_trib = 2 * nelem + nriver + pihmRTcopy->river[i].up2;   // 01.14 by Wei Zhi  // 02.21 fix RT conc index, but different for flux-pihm rivflx index
    }   
		CD->Flux[k].nodeuu = 0;  
		CD->Flux[k].nodell = 0;   
		CD->Flux[k].flux_type = 0;
    ...
    CD->Flux[k].flux_trib = 0.0;  // 01.14 by Wei Zhi  
		CD->Flux[k].distance = 1.0;
		CD->Flux[k].s_area = 0.0;
    //fprintf(stderr, "  [i=%d] CD->Flux[k, %d].nodeup, nodelo, up2 = %d, %d, %d \n", i, k, CD->Flux[k].nodeup, CD->Flux[k].nodelo, CD->Flux[k].node_trib);  // 09.28 good so far  02.21 fix bug, now OK   
		k++;
	}
...
#pragma omp parallel for
 ...
 //printf(" Unsat lateral flux: k = %d \n", k-1);
  ...
  //printf(" k = %d \n", k); 
    ...
    if (pihmFlux->river[i].up1 > 0)  // not tributary starting point
        // 02.21 fix bug, index different in RT and Flux-PIHM
        //CD->Flux[k].flux += - (pihmFlux->river[CD->Flux[k].nodelo - 1].wf.rivflow[9] * 86400 + pihmFlux->river[CD->Flux[k].nodelo - 1].wf.rivflow[1] * 86400);
    ...
    //printf(" [timelps = %.2f]: riv[%d].rivflow[10+0] = %6.4f [m3/d], up1[%d].rivflow[9+1] = %6.4f [m3/d]. \n", timelps, i+1, pihmFlux->river[i].wf.rivflow[10] * 86400 + pihmFlux->river[i].wf.rivflow[0] * 86400, CD->Flux[k].nodelo, CD->Flux[k].flux);
    
    if (CD->Flux[k].node_trib > 0)
      // 02.21 fix bug, index different in RT and Flux-PIHM
      //CD->Flux[k].flux_trib += - (pihmFlux->river[CD->Flux[k].node_trib-1].wf.rivflow[9] * 86400 + pihmFlux->river[CD->Flux[k].node_trib-1].wf.rivflow[1] * 86400);
    ...
    // 02.21
    //fprintf(stderr, "  [timelps = %.2f] riv[i, %d].upmain[up1 = %d] = %.3f [m3/d], flux_trib[up2 = %d] = %.3f [m3/d] \n", timelps, i+1, CD->Flux[k].nodelo - 2 * nelem - nriver, CD->Flux[k].flux, CD->Flux[k].node_trib - 2 * nelem - nriver, CD->Flux[k].flux_trib);       
    // 01.14 change for tributary flux
    
    //fprintf(stderr, "  [timelps = %.2f] riv[i, %d].wf.rivflow[10] = %.3f [m3/d] \n", timelps, i, (pihmFlux->riv[i].wf.rivflow[10] * 86400));
    //if (i == 37 - 1)
      //printf(" rivflx10+0 (k = %d) = %.3f [m3/d]; \n", k, CD->Flux[k].flux);
		k++;
	}
  //printf(" k = %d \n", k); 
